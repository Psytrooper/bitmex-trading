...
class BitmexTradingBot:
	...
	def __init_logger__(self):
		...
		fh = RotatingFileHandler(self.defaults.get('log.trading.outfile'), maxBytes=20000000, backupCount = 4)
	...
	def insert_fill_conditionally(self, trade):
		...
		self.logger.info(f"Inserting fill: execID:{trade['execID']} orderID:{trade['orderID']} symbol:{trade['symbol']} side:{trade['side']} price:{trade['price']} orderQty:{trade['orderQty']} ordType:{trade['ordType']} ordStatus:{trade['ordStatus']} lastPx:{trade['lastPx']} lastQty:{trade['lastQty']} leavesQty:{trade['leavesQty']} cumQty:{trade['cumQty']} avgPx:{trade['avgPx']}")
	...
	def insert_position(self, position):
		...
		self.logger.info(f"Inserting position: symbol:{position['symbol']} currentQty:{position['currentQty']} avgCostPrice:{position['avgCostPrice']} unrealisedPnl:{position['unrealisedPnl']} currentTimestamp:{now}")
		try:
			with self.connection.cursor() as cursor:
	...
	def insert_order(self, order_id, symbol, price, side, quantity, order_type, created_dt, decision_px, stop_px=None):
		self.logger.info(f'Inserting order: order_id:{order_id} symbol:{symbol} price:{price} decision_px:{decision_px} stop_px:{stop_px} side:{side} quantity:{quantity} order_type:{order_type} created_dt:{created_dt}')
		try:
			with self.connection.cursor() as cursor:
	...
	def broadcast_order_failure(self, symbol, quantity, decision_px, stop_px=None, position=None, failure_message=None):
		...
		try:
			self.sns.publish(TopicArn=self.defaults.get('sns.topic.arn'), Message=notification, Subject='Bitmex order failed')
		except Exception:
			# Capture stack trace if we cannot send out messages, but do not interrupt trading.
			self.logger.error(traceback2.format_exc())

	def broadcast_message(self, symbol, quantity, avg_px, decision_px=None, realized_pnl=None, stop_px=None, ord_status=None):
	...
	def place_stop_order(self, symbol, quantity, entry_px, decision_px):
		...
		if not new_stop_order:
			...
			self.broadcast_order_failure(symbol, quantity, decision_px, stop_px=stop_px, failure_message=failure_message)
			return StatusCode.ERROR

		# For stop orders, there is no price, just a stop-price.
		create_timestamp = new_stop_order[0]['timestamp'].timestamp()
		self.insert_order(new_stop_order[0]['orderID'], new_stop_order[0]['symbol'], new_stop_order[0]['price'], new_stop_order[0]['side'], new_stop_order[0]['orderQty'], 'Stop', create_timestamp, decision_px, stop_px=new_stop_order[0]['stopPx'])
		
		self.broadcast_message(symbol, quantity, new_stop_order[0]['avgPx'], decision_px=decision_px, stop_px=stop_px, ord_status=new_stop_order[0]['ordStatus'])
	...
	def place_order(self, symbol, quantity, decision_px, position=None):
		...
		while attempts < 5 and not new_order:
			self.logger.info(f'Attempt to place order on attempt:{attempts}')
			try:
				new_order = self.bitmex_client.Order.Order_new(symbol=symbol, orderQty=quantity, ordType='Market').result()
				break
			except bravado.exception.HTTPServiceUnavailable:
				# XXX we should check the error message to be sure the "overloaded" condition happened.
				self.logger.error(traceback2.format_exc())
		...
		if not new_order:
			...
			self.broadcast_order_failure(symbol, quantity, decision_px, position=position, failure_message=failure_message)
			return StatusCode.ERROR

		# Persist new order to data store.
		create_timestamp = new_order[0]['timestamp'].timestamp()
		self.insert_order(new_order[0]['orderID'], new_order[0]['symbol'], new_order[0]['price'], new_order[0]['side'], new_order[0]['orderQty'], 'Market', create_timestamp, decision_px)
		...
		self.broadcast_message(symbol, quantity, new_order[0]['avgPx'], decision_px=decision_px, realized_pnl=realized_pnl, ord_status=new_order[0]['ordStatus'])
	...
	def reconcile_fills_and_positions(self):
		...
		try:
			trade_history = self.bitmex_client.Execution.Execution_getTradeHistory(symbol='XBTUSD', count=64, reverse=True, filter=json.dumps({"execType": "Trade", "startTime": start_time_str})).result()
		except Exception:
			self.logger.error(traceback2.format_exc())
			return
		...
		for trade in trade_history[0]:
			...
			if trade['ordType'] == 'Stop' and (case1 or case2):
				...
				self.broadcast_message(trade['symbol'], quantity, trade['avgPx'], stop_px=trade['stopPx'], ord_status=trade['ordStatus'])
	...
	def exceeds_stop_threshold(self, last_px, decisions, side):
		...
		self.logger.info(f'Check whether stop-price reached for last_px:{last_px} side:{side} stop_threshold:{stop_threshold}')
	...
	def make_long_trading_decision(self, df):
		...
		last_idx = -1
		self.logger.info(f'df.z_180.iloc[last_idx]:{df.z_180.iloc[last_idx]} df.close_px.iloc[last_idx]:{df.close_px.iloc[last_idx]} stop threshold:{stop_threshold} mkt_price:{mkt_price}')
		...
		if df.z_180.iloc[-1] > 10 and cur_position == 0 and df.volatility_180.iloc[-2] < 0.0005:
			self.logger.info(f'Open new position only if it implies a change in our current position of {cur_position}.')
			...
			self.logger.info(f'Close position, but only if it implies a change in our current position of {cur_position}.')
	...
	def make_short_trading_decision(self, df):
		...
		self.logger.info(f'df.ma_comps.iloc[last_idx]:{df.ma_comps.iloc[last_idx]} df.close_px.iloc[last_idx]:{df.close_px.iloc[last_idx]} df.close_px.iloc[last_idx-1]:{df.close_px.iloc[last_idx-1]} df.ma_20_days.iloc[last_idx]:{df.ma_20_days.iloc[last_idx]} threshold:{threshold}')
		...
		if cur_position <= 0 \
			...
			self.logger.info(f'Close position after 3 hours, but only if it implies a change in our current position of {cur_position}.')
			code = self.resolve_positions('XBTUSD', df.close_px.iloc[last_idx], 'short') if cur_position > 0 else StatusCode.OK
			if code == StatusCode.OK:
				decision_position, decision_px = 0, 0
		else:
			self.logger.info('Hold onto current position.')
...
class BitmexTradescraper:
	...
	def __init_logger__(self):
		...
		logger.setLevel(logging.DEBUG)
		# create file handler which logs even debug messages
		# fh = logging.FileHandler(self.defaults.get('log.buckets.outfile'))
		fh = RotatingFileHandler(self.defaults.get('log.buckets.outfile'), maxBytes=20000000, backupCount = 4)
		fh.setLevel(logging.INFO)
		# create console handler with a higher log level
		ch = logging.StreamHandler()
		ch.setLevel(logging.INFO)
	...
	def insert_trade_bucket(self, trade_bucket):
		...
		try:
			with self.connection.cursor() as cursor:
				...
				self.logger.info(f'timestamp_dt:{timestamp} symbol:{symbol} open_px:{open_px} high_px:{high_px} low_px:{low_px} close_px:{close_px} trades:{trades} volume:{volume} vwap:{vwap} lastSize:{lastSize} turnover:{turnover} homeNotional:{homeNotional} foreignNotional:{foreignNotional}')
	...
	def fill_gaps(self, horizon=21*24*60*60):
		...
		while start_time <= now:
			...
			try:
				trades = self.bitmex_client.Trade.Trade_getBucketed(symbol="XBTUSD", binSize="1m", partial=False, reverse=False, count=120, filter=json.dumps({"startTime": start_time_str})).result()
				self.logger.info(f'start_time_str:{start_time_str} ntrades:{len(trades[0])}')
			except Exception:
				self.logger.error(traceback2.format_exc())
				trades = None
			...
			if trades:
				for k in range(0, len(trades[0])): self.insert_trade_bucket(trades[0][k])
...
def interrupt_handler(sif, frame):
	...
	time.sleep(1)
	bitmexscraper.gracefully_finish()
	bitmexBot.gracefully_finish()
	connection.close()
	sys.exit(0)
...
connection = init_mysql_connection(defaults) 
bitmexscraper = BitmexTradescraper(defaults, tradeSignal, connection)
bitmexBot = BitmexTradingBot(defaults, tradeSignal, connection)
signal.signal(signal.SIGINT, interrupt_handler)
signal.signal(signal.SIGTERM, interrupt_handler)
signal.signal(signal.SIGHUP, interrupt_handler)
try:
	producer = threading.Thread(target = bitmexscraper.start_scrapping)
	producer.start()
	consumer = threading.Thread(target = bitmexBot.start_trading)
	consumer.start()
