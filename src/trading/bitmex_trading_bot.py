...
from src.utils.MySqlDataStore import ConnectionWrapper
...
class BitmexTradingBot:
    ...
    def __init__(self, config, signal_queue):
        ...
        self.boto3 = AwsSnsBoto3(profile_name='bitmex-sns', logger=self.logger,
                                 topic=self.defaults.get('sns.topic.arn'))
        self.boto3.notify(message="Trading Bot Started.", subject="Bitmex trade")
    ...
    def should_place_order(self, mkt_price, decisions):
        ...
        for k in range(1, 11):
            if k > len(decisions):
                break
            if decisions[-k]['synthetic']:  # decisions[-k]['synthetic'] == True:
                continue
            if not decisions[-k]['active']:  # decisions[-k]['active'] == False:
                break
            active_count += 1
            if active_count >= 5:
                five_minute_index = k
    ...
    def broadcast_order_failure(self, symbol, quantity, decision_px, stop_px=None, position=None, failure_message=None):
        ...
        try:
            self.boto3.notify(message=notification, subject='Bitmex order failed')
        except Exception as e:
            self.logger.warning(e)
    ...
    def broadcast_message(self, symbol, quantity, avg_px, decision_px=None, realized_pnl=None, stop_px=None,
        ...
        try:
            self.boto3.notify(message=notification, subject='Bitmex trade')
        except Exception as e:
            self.logger.warning(e)
            # Capture stack trace if we cannot send out messages, but do not interrupt trading.
            self.logger.error(traceback2.format_exc())
    ...
    def conditionally_trade(self):
        now = datetime.utcnow().replace(tzinfo=timezone.utc).timestamp()
        self.logger.info(f'Trading trigger fired at {datetime.fromtimestamp(now)}')
        ...
        df = self.read_trade_buckets_into_dataframe(symbol='XBTUSD', ndays=21)
        if df.empty:
            return

        # Re-sample data, eliminating unused columns.
        df['time'] = pd.to_datetime(df.timestamp_dt)
        df['my_date'] = df.time.dt.date
        df['hour'] = df.time.dt.hour

        df.to_csv("buckets.csv")
        ...
        # one minute version use 1 min bars with no re-sampling
    ...
    def gracefully_finish(self):
        ...
        self.boto3.notify(message="Trading Bot Stopped.", subject='Bitmex trade')
